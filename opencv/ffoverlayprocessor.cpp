...
{
    adapter = new FFVideoAdapter();//定义adapter
    background = cv::Mat::zeros(1080,1920,CV_8UC3);//初始化背景
}
...
void FFOverlayProcessor::overlayImage(cv::Mat &foreground, int x, int y, int w, int h)
{
    ...
    //验证并调整前景ROI坐标
    if(fg_x1 <0||fg_y1<0||
        fg_x2>resized_foreground.cols||fg_y2>resized_foreground.rows)
    {
        ...
        if(fg_x1 >= fg_x2||fg_y1>=fg_y2){
            std::cerr<<"Adjuested foreground ROI still invalid"<<std::endl;
            return;
        }
    }
    //创建前景图的裁剪区域
