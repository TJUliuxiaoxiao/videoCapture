...
void FFRenderWidget::calcPos()
{
    ...
    //确保父控件尺寸有效
    if(parentRect.width()<=0||parentRect.height()){
        return;
    }
    //获取当前控件在父控件的位置
...
void FFRenderWidget::mousePressEvent(QMouseEvent *event)
{
    if(event->button()==Qt::LeftButton){
        this->raise();//将窗口置于父窗口最前方
        if(capWindow){
            capWindow->adjustStackNumber(type);
        }
        dragPos = event->globalPos() - this->frameGeometry().topLeft();
        initialGeometry = this->frameGeometry();
...
void FFRenderWidget::mouseMoveEvent(QMouseEvent *event)
{
    if(event->buttons()&Qt::LeftButton){
        ...
        if(resizeDir == NODIR){
            ...
            QPoint delta = event->globalPos() -
                           (dragPos+initialGeometry.topLeft());
            //计算鼠标相对于窗口初始位置的偏移
            ...
            case TOP:
                ...
                break;
            case BOTTOM:
                newGeometry.setLeft(newGeometry.bottom() + delta.y());
                break;
            case LEFT:
            ...
            case BOTTOM_LEFT:
                newGeometry.setLeft(newGeometry.left() + delta.x());
                newGeometry.setRight(newGeometry.bottom()+ delta.y());
                break;
            case BOTTOM_RIGHT:
                newGeometry.setRight(newGeometry.right() + delta.x());
            ...
            bool canResize = true;
            if((resizeDir == TOP ||
                 resizeDir == BOTTOM)&&newGeometry.height()-2<this->minimumHeight()){
...
int FFRenderWidget::getResizeDirection(const QPoint &pos)
{
