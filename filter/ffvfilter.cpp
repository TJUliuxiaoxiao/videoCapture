...
void FFVFilter::init(FFVFrameQueue *encoderFrmQueue_, FFVDecoder *vDecoder1_, FFVDecoder *vDecoder2_)
{
    std::lock_guard<std::mutex> lock(filterMutex);
...
void FFVFilter::initFilter(AVCodecContext *codecCtx1_, AVCodecContext *codecCtx2_, AVStream *stream1_, AVStream *stream2_)
{
...
void FFVFilter::createBufferFilter(AVFilterContext **ctx, AVCodecContext *codecCtx, AVStream *stream, const char *name)
{
    ...
    // codecCtx->width,codecCtx->height,AV_PIX_FMT_YUV420P,timeBase.num,timeBase.den,
    // codecCtx->sample_aspect_ratio.num,codecCtx->sample_aspect_ratio.den);
    std::ostringstream argStream;
    argStream<< "video_size=" << codecCtx->width << "x" << codecCtx->height
        << ":pix_fmt=" << AV_PIX_FMT_YUV420P
        << ":time_base=" << timeBase.num << "/" << timeBase.den
        << ":pixel_aspect=" << codecCtx->sample_aspect_ratio.num << "/" << codecCtx->sample_aspect_ratio.den;
    std::string args = argStream.str();
    int ret = avfilter_graph_create_filter(ctx,bufferFilter,name,args.c_str(),nullptr,filterGraph)    ;
    if(ret < 0 ){
        printError(ret);
...
void FFVFilter::linkFilters(AVFilterContext *src, int srcPad, AVFilterContext *dst, int dstPad)
{
    ...
    if(ret < 0){
        ...
        std::cerr<<"Failed to link filters:"
                  <<avfilter_pad_get_name(src->output_pads,srcPad)
...
void FFVFilter::createBufferSinkFilter()
{
    ...
    //新配置上下文，在设置参数
    int ret = avfilter_graph_create_filter(&bufferSinkCtx,bufferSinkFilter,"out",nullptr,nullptr,
                                           filterGraph);
    if(ret<0){
        printError(ret);
