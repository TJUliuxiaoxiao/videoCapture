...
FFAFrameQueue::FFAFrameQueue():m_stop(false){}
...
void FFAFrameQueue::enqueue(AVFrame* srcFrame){
    ...
    cond.wait(lock,[this](){
        return frmQueue.size()<MAX_PACKET_SIZE || m_stop.load();
    });
    if(m_stop.load()){
...
void FFAFrameQueue::enqueueNull(){
    std::unique_lock<std::mutex> lock(mutex);
    cond.wait(lock,[this](){return frmQueue.size()<MAX_PACKET_SIZE||m_stop.load();});
    if(m_stop.load()){
        return;
    }
...
AVFrame* FFAFrameQueue::dequeue(){
    ...
    cond.wait(lock,[this](){
        return !frmQueue.empty()||m_stop.load();
    });
    if(m_stop.load()){
        return nullptr;
    }
