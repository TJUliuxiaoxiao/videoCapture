...
void FFFaceDetector::init()
{
    ...
    faceDetector->detectMultiScale(testGray,
                                   testFaces,1.3,3,0,cv::Size(30,30));
    profileDetector->detectMultiScale(testGray,testFaces,
                                      1.3,3,0,cv::Size(30,30));
}
...
AVFrame *FFFaceDetector::detectFace(AVFrame *frame)
{
    ...
    cv::Mat scaledBgrMat;
    cv::resize(bgrMat,scaledBgrMat,cv::Size(),0.25,0.25);
    //缩小75%
    cv::Mat grayMat;
    ...
    //正脸
    faceDetector->detectMultiScale(grayMat,faces,1.3,3,0,cv::Size(50,50));//最小人脸大小
    //侧脸
    std::string text = "Face";
    if(faces.empty()){
        profileDetector->detectMultiScale(grayMat,faces,1.3,3,0,cv::Size(50,50));
        text = "Profile";
    }
    ...
    for(const auto&face:lastFaces){
        ...
        for(int y = 0;y<faceROI.rows;++y){
            ...
            for(int x = 0;x<faceROI.cols;++x){
                ...
                //计算与平均值的RGB距离
                int dist = std::abs(b-avgB)+std::abs(g-avgG)+std::abs(g-avgR);
