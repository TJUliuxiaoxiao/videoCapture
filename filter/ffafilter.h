...
public:
    FFAFilter();
    ~FFAFilter();
    void init(FFAFrameQueue *encoderFrmQueue_,
              FFADecoder* aDecoder1_,FFADecoder* aDecoder2_);
    int sendFilter(AVFrame* frame1,AVFrame* frame2,int64_t start);
    ...
    void sendFrame(AVFrame* frame,int64_t start,
                   int64_t pauseTime);
    void sendSingleFilter(AVFrame* frame,int64_t pauseTime,int type);
private:
    ...
    void createAmiFilter(AVFilterGraph* filterGraph);
